...
A React Native + Expo frontend for a private pin-collection app on Android and iOS, with a responsive web preview. **Current phase: frontend first.** You can explore and edit an isolated demo without API keys. Supabase and Cloudflare R2 integration is prepared in source and deferred until the next phase.
...
**Next phase:** start with [the integration plan and database relationships](docs/INTEGRATION-PLAN.md). It covers tables, ownership, credentials, a known backend retry issue to resolve, and live acceptance checks. The [backend reference](docs/BACKEND.md) contains draft migration/API/deployment details. No services have been deployed.
