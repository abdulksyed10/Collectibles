...
The repository contains a connected Supabase/R2 backend and a pending collection-first migration. Applying source changes does not update the live project. Start with [Connect services](CONNECT-SERVICES.md) and use the deployment sequence below.
...
1. Link the existing project, back up metadata, and inspect `supabase db push --dry-run`. For the collection-first upgrade, do not apply migrations while clients can write the old shape. Deploy `media` with `MEDIA_MUTATIONS_ENABLED=false`, drain writes, apply `202609240001_public_image_lookup_bridge.sql`, deploy the bridge-aware `public-media`, then apply `202609240002_collection_first.sql` as one transaction. It promotes former categories to collections, former collections to child categories, copies former collection visibility onto each item, and preserves item/image identities and legacy share aliases. Restore mutations only after the compatible app and privacy checks are ready.
...
Failures are JSON `{ "error":"<safe message>", "code":"<stable code>" }`. Typical statuses: 400 invalid input, 401 invalid session, 403 blocked Origin or an operator pause, 404 missing owned target, 409 photo already exists/account deletion in progress, 413 oversized body/image, 415 non-JSON, 429 a budget limit, 503 storage/database failure. Retry a failed deletion. A photo cannot be replaced: delete the item and add it again. A failed upload without a committed `item_images` row can be retried. A lost successful upload response can produce 409 on retry; refresh the item to confirm its photo.

## Ownership, quotas and serialization

Client `categories`, `collections` and `items` inserts default `owner_id` to `auth.uid()`. A collection is the parent; a category has an immutable owned `collection_id`; an item has an owned collection, optional category from that same collection, and its own private/public visibility. Clients can read their own metadata and edit permitted text and item placement/visibility. Anonymous users have no table privileges. RLS and composite foreign keys reject foreign parents and ownership reassignment. Only the server can write image rows or delete collections/items. `public.delete_category` owner-locks, detaches entries, and deletes a category without deleting entries or photos. New users start empty.

Each user has a private counter/lock row. Database triggers atomically enforce **50 categories, 50 collections and 500 items**, including direct REST inserts and concurrent requests. Client inserts and edits lock that row before acquiring metadata row locks; server media operations take locks in the same order and hold them across storage work. Reassigning an item to an owned collection therefore cannot race collection deletion. The server never trusts a supplied owner ID.
...
## Upload and read budgets

`private.media_limits` is readable and writable only from the server/operator connection. Defaults are 100 active photos per account, 20 upload attempts per account per hour, 50 per account per day, and 200 per app per day. An attempt is recorded after an owned empty item is checked but before the JPEG is decoded. Invalid images and failed R2 writes therefore count, preventing free request and retry abuse.

Every potential image pair reserves its actual bytes before R2 receives either object. The account allowance is 250 MiB and the whole app allowance is 1 GiB. Reservations survive failed writes, deletion and cleanup because an interrupted PUT can complete after the request fails. This is conservative by design: it bounds the app's lifetime beta allocation, rather than assuming deleted objects or failed requests cost nothing. Increase an allowance only after reviewing the counters and provider usage.

`public-media` checks visibility and item membership first, then consumes one of 10,000 app-wide daily public-photo reads before it asks R2 for a byte. Set `uploads_enabled=false` or `public_reads_enabled=false` in `private.media_limits` for an immediate operator pause. These controls constrain app media usage; they do not guarantee a provider-wide dollar ceiling for function invocations or traffic outside this bucket.
...
Before inviting users, verify on the real services with two disposable accounts: A cannot read/update/sign/delete B's item; anonymous owner actions and private-item image requests fail; an explicit public entry appears in the public projection, a private sibling does not, and revocation makes both metadata and photo unavailable; a category mismatch is rejected; category deletion detaches without deleting media; legacy links stay category-scoped; oversized and invalid images fail; duplicate uploads produce one image; upload versus collection/account deletion stays serialized; R2 errors leave deletions retryable; deleted keys return 404; bucket public access is disabled; signed URLs expire; account deletion removes Auth and metadata; the cleanup job runs successfully. Never use production user data for these checks.
