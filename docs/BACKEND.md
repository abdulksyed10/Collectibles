...
| --- | --- |
| `{ "action":"upload", "pinId":"<uuid>", "imageBase64":"<raw JPEG base64>", "thumbnailBase64":"<raw JPEG base64>" }` | `{ "ok":true }` |
| `{ "action":"read", "pinIds":["<uuid>"] }` | `{ "images":[{ "pinId":"<uuid>", "url":"<signed GET>", "thumbnailUrl":"<signed GET>", "expiresAt":"<ISO timestamp>" }] }` |
| `{ "action":"delete-pin", "pinId":"<uuid>" }` | `{ "ok":true }` |
...
`MEDIA_ALLOWED_ORIGINS` is a comma-separated exact list, such as `http://localhost:8081,https://pins.example.com`. Configure the R2 bucket CORS rule to allow only those web origins, methods `GET` and `HEAD`, and response header `ETag`. Uploads go through the function, so R2 does not need browser PUT permission. CORS is a browser boundary; Auth and RLS enforce ownership. Signed responses and uploaded objects use private/no-store caching directives. Client image components must use memory-only caching.
...
Before inviting users, verify on the real services with two accounts: A cannot read/update/sign/delete B's pin; anonymous requests fail; foreign collection reassignment fails; oversized and invalid images fail; duplicate uploads produce one image; upload versus collection/account deletion stays serialized; R2 errors leave deletions retryable; deleted keys return 404; bucket public access is disabled; signed URLs expire; account deletion removes Auth and metadata; the cleanup job runs successfully. Never use production user data for these checks.
