...
The Supabase schema and the `media` and `public-media` functions are deployed. Owner-account and device acceptance checks still need completion. Start with [Connect services](CONNECT-SERVICES.md).
...
1. Create a Supabase project. Link the Supabase CLI, review `supabase db push --dry-run`, then run `supabase db push`. Apply `202609150001_private_pins.sql`, `202609160001_media_attempts.sql`, `202609160002_collectibles_hierarchy.sql`, `202609160003_collection_sharing.sql`, `202609160004_media_budgets.sql`, then `202609170005_public_catalog.sql`. The hierarchy migration preserves existing records, renames pin tables/columns to generic items, and places existing collections under their owner's Pins category. The catalog migration exposes only bounded public-card metadata through an RPC; it does not relax table RLS. Deploy the matching item-based function and cleanup code after migration; the old pin-based API is incompatible. When upgrading an existing deployment, pause media traffic until the migration, updated function and cleanup script are all in place.
...
5. Deploy `supabase functions deploy media` and `supabase functions deploy public-media`. JWT verification is performed inside the function with Supabase Auth `getUser(token)`; `verify_jwt=false` avoids the legacy gateway verifier rejecting new asymmetric JWTs. It does **not** permit unauthenticated owner actions. The separate `public-media` function permits anonymous reads only after checking public collection visibility and item membership. See [collection sharing](COLLECTION-SHARING.md).
...
Before inviting users, verify on the real services with two accounts: A cannot read/update/sign/delete B's item; anonymous owner-action and private collection requests fail; explicitly shared collections expose only their public projection and revert to unavailable when made private; foreign collection reassignment fails; oversized and invalid images fail; duplicate uploads produce one image; upload versus collection/account deletion stays serialized; R2 errors leave deletions retryable; deleted keys return 404; bucket public access is disabled; signed URLs expire; account deletion removes Auth and metadata; the cleanup job runs successfully. Never use production user data for these checks.
