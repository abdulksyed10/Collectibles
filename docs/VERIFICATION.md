...
- R2 authenticated access succeeds against the empty bucket. Cloudflare dashboard checks confirm public access, custom domains and the public development URL are disabled. The configured CORS rule permits only GET/HEAD browser reads from `http://127.0.0.1:4173` and `http://localhost:8081`; a signed private-bucket request from the first origin returned the matching CORS header. Add the exact hosted origin before release.
...
- All Chrome browser scenarios pass against the isolated fixture export. Browser tests now start their own server on port 4174 rather than reusing a potentially stale connected preview on 4173. PREVIEW_DIR and PLAYWRIGHT_PORT allow isolated test exports.
...
- GitHub browser CI exposed a small-phone overlap: the search field would not shrink, letting Add item cover Clear search. `minWidth: 0` on the input and a 320px/390px regression check resolve it. The browser suite now has 14 passing scenarios.
- Replaced a date assertion that compared the database `current_date` with JavaScript's UTC calendar date. It now compares the default with the database's own date, so the sharing fixture remains valid around midnight. All 62 local tests pass.
