...
async function fakeBackend(page: Page, options: { failPhoto?: boolean; failLogoutOnce?: boolean; categoryCount?: number } = {}) {
  ...
  if (options.failPhoto) { collections = [{ id: 'travel', category_id: 'pins', owner_id: owner, name: 'Travel', description: '', created_at: now }]; items = [{ id: 'existing', owner_id: owner, collection_id: 'travel', title: 'Old name', notes: '', created_at: now, updated_at: now }]; }
...
async function exerciseHierarchy(page: Page) {
  ...
  await expect(page.getByText('Move or delete this category’s collections before deleting it.')).toBeVisible();
...
test('mobile collector can create, photograph, edit, search and delete a private item', async ({page}) => {
  ...
  await page.getByLabel('Search items').fill('missing'); await expect(page.getByText('No items found.',{exact:true})).toBeVisible();
...
test('demo works without services and resets changes when leaving', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Open Mountain memories' }).click(); await page.getByRole('button', { name: 'Edit item' }).click();
