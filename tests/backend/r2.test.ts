...
const config={accountId:'00000000000000000000000000000000',bucket:'private-items-test',accessKeyId:'test-access-id',secretAccessKey:'test-secret'};
...
test('R2 signs private GET URLs for exactly five minutes and never exposes secret credentials',async()=>{
  ...
  const url=new URL(await store.sign('owner/item/full.jpg'));
  assert.equal(url.origin,'https://00000000000000000000000000000000.r2.cloudflarestorage.com');
  assert.equal(url.pathname,'/private-items-test/owner/item/full.jpg');
...
test('uploaded JPEGs have private no-store headers, exact bytes and signed authentication',async()=>{
  ...
  await store.put('owner/item/full.jpg',new Uint8Array([1,2,3]));
  assert.ok(captured);
...
test('R2 bulk deletion checks per-object errors even with HTTP 200 and rejects HTTP failures',async()=>{
  ...
  await assert.rejects(failed.put('owner/item/full.jpg',new Uint8Array([1])),/storage/i);
});
...
test('bulk deletion splits at 1000 keys and sends signed, checksummed XML',async()=>{
  ...
  await store.remove(Array.from({length:1001},(_,i)=>`owner/item-${i}/full.jpg`));
