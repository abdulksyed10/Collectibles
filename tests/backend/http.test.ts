...
const handler=createHandler({origins:['https://items.example'],authenticate:async token=>token==='valid-session'?owner:null,handle:async(account,action)=>({account,action})});
...
test('only the authenticated account is passed to operations; responses prevent caching',async()=>{
  const response=await handler(req({authorization:'Bearer valid-session',origin:'https://items.example'}));
  assert.equal(response.status,200);
  assert.deepEqual(await response.json(),{account:owner,action:{action:'delete-account'}});
  assert.equal(response.headers.get('cache-control'),'no-store');
  assert.equal(response.headers.get('access-control-allow-origin'),'https://items.example');
});
...
test('unapproved browser origins fail; approved preflight is bounded to POST',async()=>{
  ...
  const preflight=await handler(new Request('https://function.example/media',{method:'OPTIONS',headers:{origin:'https://items.example'}}));
