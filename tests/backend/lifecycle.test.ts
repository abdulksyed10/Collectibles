...
const upload={action:'upload' as const,itemId:item,imageBase64:image,thumbnailBase64:image};
...
test('partial upload keeps durable inventory, retry succeeds, and duplicate cannot overwrite committed photo',async()=>{
  ...
  assert.equal((await pg.query('SELECT * FROM item_images')).rows.length,0);
  ...
  const result=await media.handle(a,{action:'read',itemIds:[item]}) as {images:{itemId:string;url:string;expiresAt:string}[]};
  assert.equal(result.images.length,1);
  assert.equal(result.images[0].itemId,item);
...
test('account deletion freezes writes before Auth; Auth failure allows an authenticated retry',async()=>{
  ...
  await assert.rejects(pg.query(`INSERT INTO collections(owner_id,category_id,name) VALUES ($1,$2,'Late')`,[b,categoryB]),/account deletion/);
  await assert.rejects(media.handle(b,{action:'read',itemIds:[]}),/deletion/);
  failAuth=false;
  ...
  assert.equal((await pg.query('SELECT * FROM categories WHERE owner_id=$1',[b])).rows.length,0);
});
...
test('cleanup removes late orphan writes after settling, preserves active media and keeps retirement inventory',async()=>{
  ...
  await pg.query(`INSERT INTO items(id,owner_id,collection_id,title) VALUES ($1,$2,$3,'Active')`,[active,a,ca]);
  await media.handle(a,{...upload,itemId:active});
  await pg.query(`UPDATE private.media_inventory SET created_at=now()-interval '2 days' WHERE item_id=$1`,[active]);
  assert.equal(await sweepMedia(db,store),0);
...
test('concurrent duplicate requests commit one photo; a simultaneous delete leaves no reachable or untracked image',async()=>{
  ...
  await pg.query(`INSERT INTO items(id,owner_id,collection_id,title) VALUES ($1,$2,$3,'Duplicate'),($4,$2,$3,'Racing')`,[duplicate,a,ca,racing]);
  const duplicateResults=await Promise.allSettled([media.handle(a,{...upload,itemId:duplicate}),media.handle(a,{...upload,itemId:duplicate})]);
  assert.equal(duplicateResults.filter(result=>result.status==='fulfilled').length,1);
  assert.equal((await pg.query('SELECT * FROM item_images WHERE item_id=$1',[duplicate])).rows.length,1);
  const raceResults=await Promise.allSettled([media.handle(a,{...upload,itemId:racing}),media.handle(a,{action:'delete-item',itemId:racing})]);
  assert.equal(raceResults[1].status,'fulfilled');
  assert.equal((await pg.query('SELECT * FROM items WHERE id=$1',[racing])).rows.length,0);
  assert.equal([...objects.keys()].some(key=>key.includes(racing)),false);
  assert.ok((await pg.query('SELECT deleted_at FROM private.media_inventory WHERE item_id=$1',[racing])).rows[0].deleted_at);
});
