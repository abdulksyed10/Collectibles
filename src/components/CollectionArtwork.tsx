...
import { Sparkles, BookOpen, Flower2, Moon, Star } from 'lucide-react-native';
...
export function CollectionArtwork({ compact = false }: { compact?: boolean }) {
  ...
  return <View accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={{ width: size, height: size * 0.83, alignSelf: 'center' }}>
    ...
    <View style={{ position: 'absolute', left: size * 0.12, top: size * 0.16, width: size * 0.38, height: size * 0.48, backgroundColor: '#F2B26F', borderRadius: 12, borderWidth: 5, borderColor: '#C68C42', alignItems: 'center', justifyContent: 'center', transform: [{ rotate: '-16deg' }] }}><BookOpen size={size * 0.24} color="#5A593A" strokeWidth={1.5} /></View>
    <View style={{ position: 'absolute', right: size * 0.1, top: size * 0.28, width: size * 0.39, height: size * 0.39, backgroundColor: '#476B58', borderWidth: 5, borderColor: '#D8BA77', borderRadius: size * 0.2, transform: [{ rotate: '16deg' }], alignItems: 'center', justifyContent: 'center' }}><Flower2 size={size * 0.28} color="#F4DEB5" strokeWidth={1.4} /></View>
