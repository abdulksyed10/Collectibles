...
export function LibraryScreen({ email, onExitDemo, onPreviewShared }: { email: string; onExitDemo?: () => void; onPreviewShared?: (collectionId: string) => void }) {
  ...
  function selectVisibility(next: 'private' | 'public') { setVisibility(next); setCollectionId(undefined); setSearch(''); setShareStatus(''); setShareError(''); }
  ...
  function selectCollection(value?: Collection) { setCollectionId(value?.id); if (value) setCategoryId(value.category_id); setSearch(''); }
  ...
  const header = <View style={{ paddingHorizontal: 8, paddingBottom: 16, gap: 12 }}>
    ...
    {!wide ? <View style={{ gap: 4 }}><View style={[ui.row, { justifyContent: 'space-between' }]}><Text style={ui.label}>Categories ({library.categories.length})</Text>{newCategory}</View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>{categoryFilters}</ScrollView><View style={[ui.row, { justifyContent: 'space-between' }]}><Text style={ui.label}>Collections ({visibleCollections.length})</Text>{newCollection}</View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>{collectionFilters}</ScrollView></View> : null}
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: colors.paper }}><View style={[ui.row, { paddingHorizontal: wide ? 32 : 16, minHeight: 56, justifyContent: 'space-between', borderBottomWidth: 1, borderColor: colors.line, backgroundColor: colors.card }]}><Brand small={!wide} /><Pressable accessibilityRole="button" accessibilityLabel="Account settings" onPress={() => setDialog({ type: 'settings' })} style={{ width: 48, height: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 24, backgroundColor: colors.pale }}><Text style={{ color: colors.green, fontFamily: fonts.bold }}>{email[0]?.toUpperCase() ?? 'Y'}</Text></Pressable></View>
    ...
    {dialog?.type === 'collection' ? <CollectionEditor collection={dialog.collection} categories={library.categories} initialCategory={categoryId} onClose={() => setDialog(null)} onSaved={value => { setCategoryId(value.category_id); setCollectionId(value.id); setVisibility(value.visibility); setShareStatus(''); setShareError(''); changed(); }} onDelete={() => { if (dialog.collection) setDialog({ type: 'delete-collection', collection: dialog.collection }); }} /> : null}
